...
const initThirdPartyUi = () => {
  ...
  const initRevealAnimations = () => {
    ...
    document.body.classList.add("reveal-lite");
...
const avatarInitialsFromName = (name) => {
  ...
  const firstTwo = parts
    ...
    .join("");
  const fallback = Array.from(value)[0] || "U";
  const initials = (firstTwo || fallback).slice(0, 2);
  return /^[a-z]/i.test(initials) ? initials.toUpperCase() : initials;
};

const ECONOMY_RATE_PER_HOUR = 1; // 1 Shengen per hour
const ECONOMY_MS_PER_HOUR = 60 * 60 * 1000;
const ECONOMY_MAX_HEARTBEAT_CREDIT_MS = 70 * 1000;
const WHEEL_SPIN_COOLDOWN_MS = 60 * 60 * 1000; // 1 hour

const ensureEconomy = (user) => {
  if (!user || typeof user !== "object") return { shengen: 0, activeMs: 0, lastHeartbeatAt: null, lastSpinAt: 0 };
  if (!user.economy || typeof user.economy !== "object") user.economy = {};
  const econ = user.economy;
  const shengen = Number(econ.shengen);
  econ.shengen = Number.isFinite(shengen) ? Math.max(0, Math.floor(shengen)) : 0;
  const activeMs = Number(econ.activeMs);
  econ.activeMs = Number.isFinite(activeMs) ? Math.max(0, Math.floor(activeMs)) : 0;
  econ.lastHeartbeatAt = econ.lastHeartbeatAt ? String(econ.lastHeartbeatAt) : null;
  const lastSpinAt = Number(econ.lastSpinAt);
  econ.lastSpinAt = Number.isFinite(lastSpinAt) ? Math.max(0, Math.floor(lastSpinAt)) : 0;
  if (!Array.isArray(econ.wheelHistory)) econ.wheelHistory = [];
  return econ;
};

const applyEconomyHeartbeatLocal = (user, nowMs) => {
  const econ = ensureEconomy(user);
  const lastHbMs = econ.lastHeartbeatAt ? Date.parse(econ.lastHeartbeatAt) : NaN;
  if (Number.isFinite(lastHbMs)) {
    const delta = Math.max(0, nowMs - lastHbMs);
    econ.activeMs += Math.min(delta, ECONOMY_MAX_HEARTBEAT_CREDIT_MS);
  }
  econ.lastHeartbeatAt = new Date(nowMs).toISOString();
  const earned = Math.floor(econ.activeMs / ECONOMY_MS_PER_HOUR) * ECONOMY_RATE_PER_HOUR;
  if (earned > 0) {
    econ.shengen += earned;
    econ.activeMs -= Math.floor(earned / ECONOMY_RATE_PER_HOUR) * ECONOMY_MS_PER_HOUR;
  }
  return earned;
};

const economyPayloadLocal = (user) => {
  const econ = ensureEconomy(user);
  const nextInMs = Math.max(0, ECONOMY_MS_PER_HOUR - (econ.activeMs % ECONOMY_MS_PER_HOUR));
  const nextSpinAtMs = econ.lastSpinAt ? econ.lastSpinAt + WHEEL_SPIN_COOLDOWN_MS : 0;
  return {
    shengen: econ.shengen,
    ratePerHour: ECONOMY_RATE_PER_HOUR,
    nextInMs,
    activeMs: econ.activeMs,
    wheel: {
      cooldownMs: WHEEL_SPIN_COOLDOWN_MS,
      nextSpinAt: nextSpinAtMs ? new Date(nextSpinAtMs).toISOString() : null,
      segments: [
        { label: "0", amount: 0, reward: wheelRewardForAmountLocal(0) },
        { label: "1", amount: 1, reward: wheelRewardForAmountLocal(1) },
        { label: "2", amount: 2, reward: wheelRewardForAmountLocal(2) },
        { label: "3", amount: 3, reward: wheelRewardForAmountLocal(3) },
        { label: "5", amount: 5, reward: wheelRewardForAmountLocal(5) },
        { label: "10", amount: 10, reward: wheelRewardForAmountLocal(10) },
      ],
      history: (econ.wheelHistory || []).slice(0, 10),
    },
  };
};

const wheelPickPrizeLocal = () => {
  const prizes = [
    { label: "0", amount: 0, weight: 20 },
    { label: "1", amount: 1, weight: 25 },
    { label: "2", amount: 2, weight: 20 },
    { label: "3", amount: 3, weight: 15 },
    { label: "5", amount: 5, weight: 12 },
    { label: "10", amount: 10, weight: 8 },
  ];
  const total = prizes.reduce((sum, item) => sum + item.weight, 0);
  const r = Math.floor(Math.random() * total);
  let acc = 0;
  for (let idx = 0; idx < prizes.length; idx += 1) {
    const prize = prizes[idx];
    acc += prize.weight;
    if (r < acc) return { label: prize.label, amount: prize.amount, segmentIndex: idx };
  }
  return { label: "0", amount: 0, segmentIndex: 0 };
};

function wheelRewardForAmountLocal(amount) {
  const n = Number(amount || 0) || 0;
  if (n <= 0) return { code: "none", ar: "لا يوجد مكافاة", en: "No reward" };
  if (n === 1) return { code: "badge", ar: "شارة تميز", en: "Profile badge" };
  if (n === 2) return { code: "priority", ar: "اولوية مراجعة", en: "Priority review" };
  if (n === 3) return { code: "support_ticket", ar: "تذكرة دعم مجانية", en: "Free support ticket" };
  if (n === 5) return { code: "fast_track", ar: "تسريع طلب", en: "Fast-track request" };
  return { code: "mystery", ar: "صندوق مفاجأة", en: "Mystery box" };
}

const burstConfetti = (count = 20) => {
  const layer = document.createElement("div");
  layer.className = "confetti-layer";
  const colors = ["#22c55e", "#3b82f6", "#f59e0b", "#ef4444", "#a855f7", "#14b8a6", "#eab308"];
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    piece.style.setProperty("--dx", `${Math.floor((Math.random() - 0.5) * 220)}px`);
    piece.style.setProperty("--rot", `${Math.floor(220 + Math.random() * 560)}deg`);
    piece.style.animationDuration = `${900 + Math.floor(Math.random() * 700)}ms`;
...
const pushMockAudit = (data, entry) => {
  ...
  if (data.auditLog.length > 800) data.auditLog.splice(800);
  return row;
};
...
const mockApiRequest = (path, options = {}, token) => {
  ...
  if (path === "/api/me" && method === "GET") {
    ...
    ensureEconomy(currentUser);
    const role = normalizeRole(currentUser.role);
    if (role !== currentUser.role) {
      currentUser.role = role;
      writeMockData(data);
    }
    return { user: { ...currentUser, shengen: economyPayloadLocal(currentUser).shengen } };
  }

  if (path === "/api/logout-all" && method === "POST") {
    if (!currentUser) throw new Error("Unauthorized");
    let removed = 0;
    Object.keys(sessions).forEach((tokenKey) => {
      if (sessions[tokenKey] === currentUser.id) {
  ...
  if (path === "/api/account/password" && method === "POST") {
    ...
    if (String(currentUser.password || "") !== currentPassword) throw new Error("Invalid current password");
    if (!isStrongPassword(newPassword)) throw new Error("Password must be 10+ chars with upper/lower/digit");
  ...
  if (path === "/api/audit" && method === "GET") {
    ...
    return { audit: Array.isArray(data.auditLog) ? data.auditLog.slice(0, 200) : [], limit: 200 };
  }

  if (path === "/api/economy/balance" && method === "GET") {
    if (!currentUser) throw new Error("Unauthorized");
    ensureEconomy(currentUser);
    writeMockData(data);
    return { economy: economyPayloadLocal(currentUser) };
  }

  if (path === "/api/economy/heartbeat" && method === "POST") {
    if (!currentUser) throw new Error("Unauthorized");
    const earned = applyEconomyHeartbeatLocal(currentUser, Date.now());
    writeMockData(data);
    return { earned, economy: economyPayloadLocal(currentUser) };
  }
  ...
  if (path === "/api/wheel/spin" && method === "POST") {
    ...
    ensureEconomy(currentUser);
    const now = Date.now();
    const econ = currentUser.economy;
    const nextAt = econ.lastSpinAt ? econ.lastSpinAt + WHEEL_SPIN_COOLDOWN_MS : 0;
    if (nextAt && now < nextAt) {
      const nextSpinAt = new Date(nextAt).toISOString();
      throw new Error(`Wheel cooldown until ${nextSpinAt}`);
    }
    const prize = wheelPickPrizeLocal();
    const reward = wheelRewardForAmountLocal(prize.amount);
    econ.wheelHistory.unshift({
      at: new Date(now).toISOString(),
      segmentIndex: prize.segmentIndex,
      reward: reward.code,
      amount: prize.amount,
    });
    if (econ.wheelHistory.length > 25) econ.wheelHistory.splice(25);
    econ.lastSpinAt = now;
    writeMockData(data);
    return {
      prize: { ...prize, reward },
      economy: economyPayloadLocal(currentUser),
      nextSpinAt: new Date(now + WHEEL_SPIN_COOLDOWN_MS).toISOString(),
    };
  }
  ...
  if (path.startsWith("/api/tickets/") && method === "PATCH") {
    ...
    const prevStatus = String(ticket.status || "open");
    const nextStatus = body.status === "closed" ? "closed" : "open";
    if (prevStatus !== nextStatus) {
      ticket.status = nextStatus;
      ticket.statusHistory.unshift({ at: nowIso, by: currentUser.id, from: prevStatus, to: nextStatus });
      if (ticket.statusHistory.length > 50) ticket.statusHistory.splice(50);
    }

    if (body.assignedTo) {
      ticket.assignedTo = String(body.assignedTo) === "me" ? currentUser.id : String(body.assignedTo);
    }
    const note = String(body.note || "").trim();
    if (note) {
      ticket.internalNotes.unshift({ at: nowIso, by: currentUser.id, text: note.slice(0, 500) });
      if (ticket.internalNotes.length > 50) ticket.internalNotes.splice(50);
    }
    ...
    pushMockAudit(data, {
      ...
      targetType: "ticket",
      targetId: ticket.id,
      details: { status: ticket.status, assignedTo: ticket.assignedTo || null, noted: Boolean(note) },
    });
    pushMockAdminNotification(data, {
  ...
  if (path === "/api/orders" && method === "POST") {
    ...
    const isOnline = wallet === "online_card";
    if (!game || !playerId || !wallet || !Number.isFinite(amount) || amount <= 0) {
      throw new Error("Missing required fields");
    }
    if (!isOnline && (!sender || !paymentRef || !proofUrl)) {
      throw new Error("Missing required fields");
    }
    ...
    const order = {
      ...
      amount,
      wallet,
      sender: isOnline ? null : sender,
      paymentRef: isOnline ? null : paymentRef,
      proofUrl: isOnline ? "" : proofUrl,
    ...
    if (isOnline) {
      // Mock: create a "payment" record and return a fake checkout URL.
      ...
      const payment = {
        ...
        amount: order.amount,
        currency: "egp",
        stripeSessionId: makeId(),
        checkoutUrl: "about:blank",
        events: [{ at: new Date().toISOString(), type: "mock", status: "paid", details: {} }],
      };
      ...
      writeMockData(data);
      return { order, payment, checkoutUrl: payment.checkoutUrl };
    }
    writeMockData(data);
    return { order };
  }
  ...
  if (path.startsWith("/api/orders/") && method === "PATCH") {
    ...
    if (hasStatus) {
      ...
      if (prevStatus !== order.status) {
        order.statusHistory.unshift({ at: nowIso, by: currentUser.id, from: prevStatus, to: order.status });
        if (order.statusHistory.length > 50) order.statusHistory.splice(50);
      }
    }
    if (body.assignedTo) {
      order.assignedTo = String(body.assignedTo) === "me" ? currentUser.id : String(body.assignedTo);
    }
    const note = String(body.note || "").trim();
    if (note) {
      order.internalNotes.unshift({ at: nowIso, by: currentUser.id, text: note.slice(0, 500) });
      if (order.internalNotes.length > 50) order.internalNotes.splice(50);
    }
    order.reviewedAt = nowIso;
    order.reviewedBy = currentUser.id;
    pushMockAudit(data, {
      actorId: currentUser.id,
      actorEmail: currentUser.email,
      actorRole: normalizeRole(currentUser.role),
      ...
      targetType: "order",
      targetId: order.id,
      details: { status: order.status, assignedTo: order.assignedTo || null, noted: Boolean(note), statusChanged: hasStatus },
    });
    pushMockAdminNotification(data, {
  ...
  if (path === "/api/payments" && method === "GET") {
    if (!currentUser) throw new Error("Unauthorized");
    data.payments = Array.isArray(data.payments) ? data.payments : [];
    const payments = isStaffRole(currentUser.role)
      ? data.payments
      : data.payments.filter((p) => String(p.userId || "") === String(currentUser.id));
    return { payments };
  }
  ...
  if (path === "/api/payments/checkout" && method === "POST") {
    ...
    const order = data.orders.find((o) => String(o.id) === orderId);
    if (!order) throw new Error("Order not found");
    const payment = (data.payments || []).find((p) => String(p.orderId || "") === String(orderId)) || null;
    return { payment, checkoutUrl: payment?.checkoutUrl || "about:blank" };
  }
...
document.querySelectorAll("[data-action='logout-all']").forEach((button) => {
  button.addEventListener("click", async () => {
    try {
      await apiRequest("/api/logout-all", { method: "POST" });
    } catch (_) {
...
if (page === "dashboard") {
  ...
  const shengenBalanceEl = document.getElementById("shengen-balance");
  const timeOnlineEl = document.getElementById("time-online");
  const nextShengenEl = document.getElementById("next-shengen");
  const wheelEl = document.querySelector("#economy .wheel");
  const wheelSpinBtn = document.getElementById("wheel-spin-btn");
  const wheelSpinStatus = document.getElementById("wheel-spin-status");
  ...
  const manualFieldsWrap = document.getElementById("order-manual-fields");
  const onlineNote = document.getElementById("order-online-note");
  ...
  const syncOrderPaymentMode = () => {
    const wallet = normalizeWallet(orderWalletSelect?.value || "");
    const isOnline = wallet === "online_card";
    if (manualFieldsWrap) manualFieldsWrap.style.display = isOnline ? "none" : "";
    if (onlineNote) onlineNote.style.display = isOnline ? "" : "none";
    setRequired(document.getElementById("order-sender"), !isOnline);
    setRequired(document.getElementById("order-ref"), !isOnline);
    setRequired(document.getElementById("order-proof-image"), !isOnline);
  };

  const syncOpenWalletPaymentLink = () => {
    if (!openWalletPaymentLink) return;
    const wallet = normalizeWallet(orderWalletSelect?.value || "");
    if (wallet === "online_card") {
  ...
  orderWalletSelect?.addEventListener("change", () => {
    syncOrderPaymentMode();
    syncOpenWalletPaymentLink();
  });
  ...
  const updateUserLabels = (user) => {
    ...
    if (userAvatar) userAvatar.textContent = avatarInitialsFromName(user.name || "");
    if (adminLink) {
  ...
  const formatHhMmSs = (ms) => formatClock(ms);

  const applyEconomyUi = (economy) => {
    if (!economy) return;
    if (shengenBalanceEl) shengenBalanceEl.textContent = String(Number(economy.shengen || 0) || 0);
    if (nextShengenEl) nextShengenEl.textContent = formatMmSs(Number(economy.nextInMs || 0));
    if (wheelLegend && economy.wheel?.segments) {
      renderWheelLegend(economy.wheel.segments, null);
    }
  };

  const wheelLegend = document.getElementById("wheel-legend");
  const rewardIconByCode = {
    none: "assets/reward-none.svg",
    badge: "assets/reward-badge.svg",
    priority: "assets/reward-priority.svg",
    support_ticket: "assets/reward-ticket.svg",
    fast_track: "assets/reward-fast.svg",
    mystery: "assets/reward-mystery.svg",
  };
  const renderWheelLegend = (segments, activeIndex) => {
    if (!wheelLegend || !Array.isArray(segments)) return;
    wheelLegend.innerHTML = "";
    segments.forEach((seg, idx) => {
      const chip = document.createElement("span");
      chip.className = `wheel-chip${idx === activeIndex ? " active" : ""}`;
      const title = seg?.reward ? uiText(seg.reward.ar, seg.reward.en) : String(seg.label || "");
      const icon = rewardIconByCode[String(seg?.reward?.code || "none")] || rewardIconByCode.none;
      chip.innerHTML = `<img src="${icon}" alt="" aria-hidden="true" /><strong>${title || "-"}</strong>`;
      wheelLegend.appendChild(chip);
    });
  };

  let wheelCooldownTimer = null;
  let wheelCooldownAtMs = 0;
  const updateWheelCooldownUi = (nextSpinAt) => {
    ...
    const at = Date.parse(String(nextSpinAt));
    if (!Number.isFinite(at)) {
      wheelSpinBtn.disabled = false;
      return;
    }
    if (wheelCooldownAtMs === at && wheelCooldownTimer) return;
    if (wheelCooldownTimer) {
      clearInterval(wheelCooldownTimer);
      wheelCooldownTimer = null;
    }
    wheelCooldownAtMs = at;
    const now = Date.now();
    ...
    if (wheelSpinStatus) {
      wheelSpinStatus.textContent = uiText(
        `المحاولة القادمة بعد ${formatHhMmSs(remaining)}.`,
        `Next spin in ${formatHhMmSs(remaining)}.`
      );
    }
    wheelCooldownTimer = setInterval(() => {
      ...
      if (left <= 0) {
        ...
        wheelCooldownTimer = null;
        wheelSpinBtn.disabled = false;
        if (wheelSpinStatus) wheelSpinStatus.textContent = uiText("متاحة الآن.", "Available now.");
        return;
      }
      if (wheelSpinStatus) {
        wheelSpinStatus.textContent = uiText(
          `المحاولة القادمة بعد ${formatHhMmSs(left)}.`,
          `Next spin in ${formatHhMmSs(left)}.`
        );
      }
    }, 1000);
  };

  const spinWheelToIndex = (index, count) => {
    if (!wheelEl) return;
    const segments = Math.max(2, Number(count) || 6);
    const idx = Math.max(0, Math.min(segments - 1, Number(index) || 0));
    const degPer = 360 / segments;
    const randOffset = (Math.random() - 0.5) * (degPer * 0.65);
    const target = 360 * 6 + (360 - (idx * degPer + degPer / 2)) + randOffset;
    wheelEl.classList.remove("spinning");
    wheelEl.style.transform = `rotate(${target}deg)`;
  };

  const sessionKey = "gl_session_opened_at";
  let openedAt = Number(sessionStorage.getItem(sessionKey) || 0);
  if (!Number.isFinite(openedAt) || openedAt <= 0) {
    openedAt = Date.now();
    sessionStorage.setItem(sessionKey, String(openedAt));
  }
  if (timeOnlineEl) {
    timeOnlineEl.textContent = formatClock(Date.now() - openedAt);
    setInterval(() => {
      timeOnlineEl.textContent = formatClock(Date.now() - openedAt);
    }, 1000);
  }

  const tickEconomy = async () => {
    const response = await apiRequest("/api/economy/heartbeat", { method: "POST" });
    applyEconomyUi(response.economy);
    updateWheelCooldownUi(response.economy?.wheel?.nextSpinAt || null);
    const gained = Number(response.earned || 0) || 0;
    if (gained > 0) {
      toast(uiText(`تم إضافة ${gained} شنغن لرصيدك.`, `+${gained} Shengen added to your balance.`), "success");
    }
  };

  // First load, then keep a lightweight heartbeat running while the dashboard is open.
  tickEconomy().catch(() => {});
  setInterval(() => tickEconomy().catch(() => {}), 60 * 1000);

  wheelSpinBtn?.addEventListener("click", async () => {
    const unlock = lockButton(wheelSpinBtn, uiText("جارٍ اللف...", "Spinning..."));
    if (wheelSpinStatus) wheelSpinStatus.textContent = "";
    if (wheelResult) wheelResult.textContent = "";
    try {
      const response = await apiRequest("/api/wheel/spin", { method: "POST" });
      applyEconomyUi(response.economy);
      const segIdx = Number(response.prize?.segmentIndex);
      const segments = response.economy?.wheel?.segments;
      if (Array.isArray(segments)) renderWheelLegend(segments, Number.isFinite(segIdx) ? segIdx : null);
      if (Number.isFinite(segIdx)) spinWheelToIndex(segIdx, Array.isArray(segments) ? segments.length : 6);
      const reward = response.prize?.reward;
      const rewardCode = String(reward?.code || "");
      if (rewardCode && rewardCode !== "none") {
        document.getElementById("economy")?.classList.add("economy-win");
        burstConfetti(26);
        setTimeout(() => document.getElementById("economy")?.classList.remove("economy-win"), 650);
      }
      if (wheelResult) {
        const title = reward ? uiText(reward.ar, reward.en) : uiText("مكافأة", "Reward");
        const icon = rewardIconByCode[String(reward?.code || "none")] || rewardIconByCode.none;
        wheelResult.innerHTML = `<span class="wheel-result-line"><img src="${icon}" alt="" aria-hidden="true" /><span>${uiText(
          `نتيجة العجلة: ${title}.`,
          `Wheel result: ${title}.`
        )}</span></span>`;
      }
      if (wheelSpinStatus && response.nextSpinAt) {
        wheelSpinStatus.textContent = uiText(
          `المحاولة القادمة: ${new Date(response.nextSpinAt).toLocaleString()}`,
          `Next spin: ${new Date(response.nextSpinAt).toLocaleString()}`
        );
      }
      updateWheelCooldownUi(response.nextSpinAt || response.economy?.wheel?.nextSpinAt || null);
    } catch (error) {
      const msg = String(error?.message || uiText("فشل لف العجلة", "Wheel spin failed"));
      if (wheelSpinStatus) wheelSpinStatus.textContent = msg;
      toast(msg, "error");
    } finally {
      unlock();
    }
      ...
      order.status === "approved"
        ...
        : order.status === "paid"
          ...
          : order.status === "pending_payment"
            ? uiText("بانتظار الدفع", "awaiting payment")
  ...
  document.querySelectorAll("[data-open]").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      if (target === "password" && passwordModal) {
  ...
  passwordForm?.addEventListener("submit", async (event) => {
    event.preventDefault();
    const currentPassword = String(passwordCurrent?.value || "");
    const newPassword = String(passwordNext?.value || "");
    const confirm = String(passwordConfirm?.value || "");
    if (!currentPassword || !newPassword) {
      if (passwordStatus) passwordStatus.textContent = uiText("يرجى إدخال الحقول المطلوبة.", "Please fill required fields.");
      return;
    }
    if (newPassword !== confirm) {
      if (passwordStatus) passwordStatus.textContent = uiText("كلمتا المرور غير متطابقتين.", "Passwords do not match.");
      return;
    }
    if (passwordStatus) passwordStatus.textContent = uiText("جارٍ التحديث...", "Updating...");
    try {
      await apiRequest("/api/account/password", {
        method: "POST",
        body: JSON.stringify({ currentPassword, newPassword }),
      });
      if (passwordStatus) passwordStatus.textContent = uiText("تم تحديث كلمة المرور.", "Password updated.");
      setTimeout(() => {
        if (passwordModal) closeModal(passwordModal);
      }, 700);
    } catch (error) {
      if (passwordStatus) {
        passwordStatus.textContent = `${uiText("فشل تحديث كلمة المرور", "Password update failed")}: ${
          error.message || uiText("خطأ في الطلب", "request error")
        }`;
      }
    }
  ...
  orderForm?.addEventListener("submit", async (event) => {
    ...
    if (!isOnline && (!sender || !paymentRef)) {
      alert(uiText("يرجى إدخال بيانات الدفع.", "Please enter payment details."));
      unlock();
      return;
    }
    if (!isOnline && !proofFile) {
      alert("صورة إثبات الدفع مطلوبة / Payment proof image is required");
      unlock();
      return;
    }
    ...
    if (!isOnline) {
      try {
        ...
        alert(
          `${uiText("فشل رفع إثبات الدفع", "Payment proof upload failed")}: ${error.message || uiText("فشل الطلب", "request failed")}`
        );
        unlock();
        return;
      }
    ...
    try {
      const response = await apiRequest("/api/orders", {
        method: "POST",
        body: JSON.stringify(isOnline ? { game, playerId, amount, wallet } : { game, playerId, amount, wallet, sender, paymentRef, proofUrl }),
      });
      renderOrder(response.order);
      if (isOnline) {
        const url = response.checkoutUrl || response.payment?.checkoutUrl;
        if (url) {
          window.open(url, "_blank", "noopener");
          alert(uiText("تم إنشاء رابط الدفع. أكمل الدفع ثم عد للمتابعة.", "Payment link created. Complete payment then come back."));
        } else {
          alert(uiText("تم إنشاء الطلب ولكن تعذر إنشاء رابط الدفع الآن.", "Order created but payment link could not be generated now."));
        }
      }
      orderForm.reset();
...
if (page === "admin") {
  ...
  const adminAuditRefreshBtn = document.getElementById("admin-audit-refresh");
  const adminPaymentList = document.getElementById("admin-payment-list");
  const adminPaymentsRefreshBtn = document.getElementById("admin-payments-refresh");
  const userName = document.querySelector(".user-name");
  const userRole = document.querySelector(".user-role");
  const userAvatar = document.getElementById("user-avatar");
  ...
  const updateAdminOverview = () => {
    ...
    const pendingOrders = ordersInRange.filter((item) =>
      ["pending", "paid", "pending_payment"].includes(String(item.status || "pending"))
    ).length;
  ...
  const ensureStaffSession = async () => {
    try {
      ...
      if (userAvatar) userAvatar.textContent = avatarInitialsFromName(me.user.name || "Staff");
    } catch (error) {
      clearSession();
      window.location.href = "index.html";
  ...
  const renderAdminTicket = (ticket) => {
    ...
    const meta = document.createElement("p");
    meta.className = "muted";
    const noteCount = Array.isArray(ticket.internalNotes) ? ticket.internalNotes.length : 0;
    const assigned = ticket.assignedTo ? `${t("مُسند", "assigned")}: ${String(ticket.assignedTo).slice(0, 8)}…` : t("غير مُسند", "unassigned");
    meta.textContent = `${ticket.priority} | ${assigned} | ${t("ملاحظات", "notes")}: ${noteCount}`;
    left.append(title, meta);
    ...
    statusBtn.className = `status-btn ${ticket.status === "closed" ? "status-closed" : ""}`;
    statusBtn.dataset.status = ticket.status;
    statusBtn.dataset.id = ticket.id;
    statusBtn.textContent = ticket.status === "closed" ? t("مغلق", "Closed") : t("مفتوح", "Open");

    const assignBtn = document.createElement("button");
    assignBtn.className = "outline-btn";
    assignBtn.type = "button";
    assignBtn.dataset.ticketAssignId = ticket.id;
    assignBtn.textContent = t("اسناد لي", "Assign to me");

    const noteBtn = document.createElement("button");
    noteBtn.className = "outline-btn";
    noteBtn.type = "button";
    noteBtn.dataset.ticketNoteId = ticket.id;
    noteBtn.textContent = t("ملاحظة", "Add note");

    actions.append(statusBtn, assignBtn, noteBtn);
  ...
  const renderAdminOrder = (order) => {
    ...
    const assigned = order.assignedTo ? `${t("مُسند", "assigned")}: ${String(order.assignedTo).slice(0, 8)}…` : t("غير مُسند", "unassigned");
    meta.textContent = `${order.amount} EGP | ${walletLabel(order.wallet)} | ref: ${order.paymentRef || "-"} | ${assigned} | ${t("ملاحظات", "notes")}: ${noteCount}`;
    left.append(title, meta);
    if (order.proofUrl) {
      const image = document.createElement("img");
      image.src = order.proofUrl;
    ...
    assignBtn.className = "outline-btn";
    assignBtn.type = "button";
    assignBtn.dataset.orderAssignId = order.id;
    assignBtn.textContent = t("اسناد لي", "Assign to me");

    const noteBtn = document.createElement("button");
    noteBtn.className = "outline-btn";
    noteBtn.type = "button";
    noteBtn.dataset.orderNoteId = order.id;
    noteBtn.textContent = t("ملاحظة", "Add note");

    actions.append(status, approve, reject, assignBtn, noteBtn);
    row.append(left, actions);
  ...
  const renderAdminPayment = (payment) => {
    if (!adminPaymentList) return;
    const row = document.createElement("div");
    row.className = "ticket-item";
    const left = document.createElement("div");
    const title = document.createElement("h4");
    title.textContent = `${String(payment.provider || "payment").toUpperCase()} | ${payment.amount ?? "-"} ${String(payment.currency || "").toUpperCase()}`;
    const meta = document.createElement("p");
    meta.className = "muted";
    meta.textContent = `status: ${payment.status || "-"} | order: ${payment.orderId || "-"} | user: ${payment.userId || "-"}`;
    left.append(title, meta);
    const right = document.createElement("span");
    right.className = "status-pill";
    right.textContent = String(payment.status || "-");
  ...
  const loadAdminPayments = async () => {
    ...
    payments.forEach(renderAdminPayment);
    if (payments.length === 0) {
      const empty = document.createElement("p");
      empty.className = "muted";
      empty.textContent = t("لا توجد مدفوعات بعد.", "No payments yet.");
      adminPaymentList.appendChild(empty);
    }
  };
  ...
  const loadAdminAudit = async () => {
    ...
    const items = Array.isArray(response.audit) ? response.audit : [];
    ...
    items.forEach((item) => {
      ...
      const left = document.createElement("div");
      const title = document.createElement("h4");
      title.textContent = `${item.action || "event"}${item.targetType ? ` | ${item.targetType}` : ""}`;
      const meta = document.createElement("p");
      meta.className = "muted";
      ...
      meta.textContent = `${at} | ${actor}${target ? ` | ${target}` : ""}`;
      left.append(title, meta);
      row.append(left);
    ...
    if (items.length === 0) {
      const empty = document.createElement("p");
      empty.className = "muted";
      empty.textContent = t("لا توجد سجلات تدقيق بعد.", "No audit entries yet.");
      adminAuditList.appendChild(empty);
    }
  };
  ...
  adminTicketList?.addEventListener("click", async (event) => {
    const assign = event.target.closest("[data-ticket-assign-id]");
    if (assign) {
      const id = assign.dataset.ticketAssignId;
      try {
        ...
        alert(t("فشل الاسناد", "Assign failed"));
      }
      return;
    }

    const noteBtn = event.target.closest("[data-ticket-note-id]");
    if (noteBtn) {
      const id = noteBtn.dataset.ticketNoteId;
      const note = prompt(t("اكتب ملاحظة داخلية للتذكرة:", "Write an internal note for the ticket:"));
      if (!note) return;
      try {
        await apiRequest(`/api/tickets/${id}`, {
          method: "PATCH",
        ...
        alert(t("فشل إضافة الملاحظة", "Add note failed"));
      }
      return;
    }
  ...
  adminOrderList?.addEventListener("click", async (event) => {
    ...
    const assign = event.target.closest("[data-order-assign-id]");
    if (assign) {
      const id = assign.dataset.orderAssignId;
      try {
    ...
    const noteBtn = event.target.closest("[data-order-note-id]");
    if (noteBtn) {
      const id = noteBtn.dataset.orderNoteId;
      const note = prompt(t("اكتب ملاحظة داخلية للطلب:", "Write an internal note for the order:"));
      if (!note) return;
      try {
        await apiRequest(`/api/orders/${id}`, {
          method: "PATCH",
  ...
  adminPaymentsRefreshBtn?.addEventListener("click", async () => {
    try {
      await loadAdminPayments();
    } catch (_) {
      alert(t("فشل تحميل المدفوعات", "Payments load failed"));
    }
  });

  adminAuditRefreshBtn?.addEventListener("click", async () => {
    try {
      await loadAdminAudit();
    } catch (error) {
      alert(t("فشل تحميل سجل التدقيق", "Audit load failed"));
    }
  });
