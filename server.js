...
const DATA_FILE = path.join(__dirname, String(process.env.DATA_JSON_FILE || "data.json"));
...
const apiRateWindowMs = Math.max(1000, Number(process.env.API_RATE_WINDOW_MS || 15 * 60 * 1000));
const apiRateMax = Math.max(30, Number(process.env.API_RATE_MAX || 300));
...
const buildAiInsights = (data, user = null) => {
  ...
  const pendingOrders = data.orders.filter((o) => ["pending", "paid", "pending_payment"].includes(o.status)).length;
...
const AUDIT_LOG_MAX = Math.max(100, Math.floor(Number(process.env.AUDIT_LOG_MAX || 1500)));
const clientIp = (req) => {
  if (TRUST_PROXY) {
...
const pushAuditLog = (data, entry) => {
  ...
  if (data.auditLog.length > AUDIT_LOG_MAX) data.auditLog.splice(AUDIT_LOG_MAX);
  return row;
};
...
const PAYMENTS_PROVIDER = String(process.env.PAYMENTS_PROVIDER || "stripe").trim().toLowerCase();
const PAYMENTS_ENABLED = String(process.env.PAYMENTS_ENABLED || "false").trim().toLowerCase() === "true";
const PAYMENT_CURRENCY = String(process.env.PAYMENT_CURRENCY || "egp").trim().toLowerCase();
...
const toMinorAmount = (amount, currency = "egp") => {
  const n = Number(amount);
  if (!Number.isFinite(n) || n <= 0) return null;
  // Stripe expects the "smallest currency unit". EGP is 2 decimals.
  const minor = Math.round(n * 100);
  if (!Number.isFinite(minor) || minor <= 0) return null;
  return minor;
};

const stripeAuthHeader = (secretKey) =>
  `Basic ${Buffer.from(`${secretKey}:`, "utf8").toString("base64")}`;

const stripeRequest = async (path, formFields) => {
  ...
  const response = await fetch(`https://api.stripe.com${path}`, {
    method: "POST",
    headers: {
      Authorization: stripeAuthHeader(STRIPE_SECRET_KEY),
  ...
  if (!response.ok) {
    const message = payload?.error?.message || payload?.error || `Stripe HTTP ${response.status}`;
    throw new Error(message);
  }
  return payload;
};

const createStripeCheckoutSession = async ({ order, user }) => {
  if (!STRIPE_SUCCESS_URL || !STRIPE_CANCEL_URL) {
    throw new Error("Stripe URLs are not configured (STRIPE_SUCCESS_URL/STRIPE_CANCEL_URL)");
  }
  const minor = toMinorAmount(order.amount, PAYMENT_CURRENCY);
  if (!minor) throw new Error("Invalid amount");
  const title = `Top-Up: ${order.game} (${order.playerId})`;
...
const verifyStripeWebhook = ({ rawBody, signatureHeader, secret, toleranceSec = 300 }) => {
  ...
  const ts = Number((kv.t || [])[0]);
  if (!Number.isFinite(ts)) throw new Error("Missing Stripe webhook timestamp");
  const nowSec = Math.floor(Date.now() / 1000);
  if (Math.abs(nowSec - ts) > toleranceSec) throw new Error("Stale Stripe webhook");
  const payload = Buffer.isBuffer(rawBody) ? rawBody : Buffer.from(String(rawBody || ""), "utf8");
  const signed = `${ts}.${payload.toString("utf8")}`;
  const expected = crypto.createHmac("sha256", secret).update(signed, "utf8").digest("hex");
  const sigs = kv.v1 || [];
  const ok = sigs.some((sig) => safeEqual(sig, expected));
  if (!ok) throw new Error("Invalid Stripe signature");
  return true;
};

const ECONOMY_MS_PER_HOUR = 60 * 60 * 1000;
// Heartbeat runs on an interval; credit at most ~1 minute per heartbeat to avoid large jumps.
const ECONOMY_MAX_HEARTBEAT_CREDIT_MS = Math.max(
  10 * 1000,
  Math.min(5 * 60 * 1000, Number(process.env.ECONOMY_MAX_HEARTBEAT_CREDIT_MS || 70 * 1000))
);
const ECONOMY_RATE_PER_HOUR = Math.max(0, Math.floor(Number(process.env.ECONOMY_RATE_PER_HOUR || 1))); // 1 shengen per hour
const WHEEL_SPIN_COOLDOWN_MS = Math.max(
  0,
  Math.floor(Number(process.env.WHEEL_SPIN_COOLDOWN_MS || 60 * 60 * 1000))
);
...
const parseWheelSegmentsEnv = (raw) => {
  const input = String(raw || "").trim();
  if (!input) return defaultWheelSegments();
  const pairs = input.split(",").map((chunk) => chunk.trim()).filter(Boolean);
  const segments = [];
  for (const pair of pairs) {
    const parts = pair.split(":").map((p) => p.trim());
    if (parts.length !== 2) return defaultWheelSegments();
    const amount = Number(parts[0]);
    const weight = Number(parts[1]);
    if (!Number.isFinite(amount) || amount < 0) return defaultWheelSegments();
    if (!Number.isFinite(weight) || weight <= 0) return defaultWheelSegments();
    segments.push({ label: String(amount), amount: Math.floor(amount), weight: Math.floor(weight) });
  }
  // Keep 6 segments to match the current UI rendering.
  if (segments.length !== 6) return defaultWheelSegments();
  return segments;
};

const getUserEconomy = (user) => {
...
const wheelRewardForAmount = (amount) => {
...
const wheelSegments = () => parseWheelSegmentsEnv(process.env.WHEEL_SEGMENTS);

const applyEconomyHeartbeat = (user, nowMs) => {
  const econ = getUserEconomy(user);
...
const economyPayload = (user) => {
  ...
  return {
    ...
    wheel: {
      ...
      segments: wheelSegments().map((seg) => ({
        label: seg.label,
        amount: seg.amount,
        reward: wheelRewardForAmount(seg.amount),
      })),
...
const wheelPickPrize = () => {
  ...
  for (let idx = 0; idx < segments.length; idx += 1) {
    ...
    acc += seg.weight;
    if (r < acc) return { label: seg.label, amount: seg.amount, segmentIndex: idx };
  }
...
const apiRateKey = (req) => {
  const ip = clientIp(req) || "unknown";
  const method = String(req.method || "GET").toUpperCase();
  const pathPart = String(req.path || "/").split("?")[0];
  return `${ip}|${method}|${pathPart}`;
};
const enforceApiRateLimit = (req) => {
  const key = apiRateKey(req);
  const now = Date.now();
  ...
  current.count += 1;
  if (current.count > apiRateMax) return true;
  return false;
};
const purgeExpiredApiRate = () => {
  const now = Date.now();
  apiRate.forEach((entry, key) => {
    if (!entry || now > entry.resetAt) apiRate.delete(key);
  });
};
setInterval(purgeExpiredApiRate, 5 * 60 * 1000).unref();

app.use((req, res, next) => {
  if (!req.path.startsWith("/api/")) return next();
  if (req.path === "/api/health") return next();
  if (req.path === "/api/public/assistant") return next();
  if (enforceApiRateLimit(req)) {
    res.setHeader("Retry-After", String(Math.ceil(apiRateWindowMs / 1000)));
  ...
  app.use(
    express.static(DIST_DIR, {
      ...
      setHeaders: (res, filePath) => {
        ...
        const isHashed = /\.[a-f0-9]{10}\.(js|css)$/i.test(base) || /^(vendor|assets)[\\/]/i.test(filePath);
        if (isHashed) {
          res.setHeader("Cache-Control", "public, max-age=31536000, immutable");
          return;
        }
        if (/\.html$/i.test(base) || base === "manifest.json") {
...
const resolvePagePath = (name) => {
  if (HAS_DIST) {
    const candidate = path.join(DIST_DIR, name);
    if (fs.existsSync(candidate)) return candidate;
  }
  return path.join(__dirname, name);
};
const sendPage = (name) => (req, res) => res.sendFile(resolvePagePath(name));
[
  "index.html",
  "landing.html",
  "dashboard.html",
...
app.post("/api/signup", (req, res) => {
  ...
  setSessionCookie(req, res, token);
  res.json({
    user: { id: user.id, name: user.name, email: user.email, role: user.role, shengen: economyPayload(user).shengen },
  });
});
...
app.post("/api/login", (req, res) => {
  ...
  setSessionCookie(req, res, token);
  res.json({
    user: { id: user.id, name: user.name, email: user.email, role: normalizeRole(user.role), shengen: economyPayload(user).shengen },
  });
});
...
app.post("/api/logout", (req, res) => {
  const sessionEntry = getSession(req);
  if (sessionEntry) {
    sessions.delete(sessionEntry.token);
    const data = readData();
    const user = data.users.find((u) => u.id === sessionEntry.session.userId);
    if (user) {
      pushAuditLog(data, {
        actorId: user.id,
        actorEmail: user.email,
...
app.get("/api/payments", requireAuth, (req, res) => {
  ...
  const payments = isStaff(req.user)
    ? data.payments
    : data.payments.filter((p) => String(p.userId || "") === String(req.user.id));
  res.json({ payments });
});
...
app.post("/api/payments/checkout", requireAuth, async (req, res) => {
  ...
  if (!PAYMENTS_ENABLED) return res.status(400).json({ error: "Payments are disabled" });
  if (PAYMENTS_PROVIDER !== "stripe") return res.status(400).json({ error: "Unsupported provider" });

  const data = readData();
  data.payments = Array.isArray(data.payments) ? data.payments : [];
  const order = data.orders.find((o) => o.id === orderId);
  if (!order) return res.status(404).json({ error: "Order not found" });
  if (!isStaff(req.user) && String(order.userId) !== String(req.user.id)) {
    return res.status(403).json({ error: "Forbidden" });
  }
  if (String(order.wallet || "") !== "online_card") {
    return res.status(400).json({ error: "Order is not an online_card order" });
  }

  const session = await createStripeCheckoutSession({ order, user: req.user });
  const payment = {
    id: crypto.randomUUID(),
    provider: "stripe",
    status: "created",
  ...
  pushAuditLog(data, {
    ...
    targetType: "payment",
    targetId: payment.id,
    details: { orderId: order.id, provider: "stripe", via: "manual_checkout" },
  });
  writeData(data);
  res.json({ payment, checkoutUrl: session.url });
});
...
app.post("/api/webhooks/stripe", async (req, res) => {
  ...
  const orderId = String(obj?.metadata?.order_id || obj?.client_reference_id || "");
  const payment = data.payments.find((p) => String(p.stripeSessionId || "") === sessionId) || null;
  const order = orderId ? data.orders.find((o) => String(o.id) === orderId) : null;

  const appendEvent = (p, status, details = {}) => {
    if (!p) return;
    ...
    if (p.events.length > 50) p.events.splice(50);
  };
  ...
  if (type === "checkout.session.completed") {
    if (payment) {
      ...
      appendEvent(payment, "paid", { orderId: order?.id || orderId });
    }
    if (order) {
      order.status = "paid";
      order.paidAt = new Date().toISOString();
      if (payment) order.paymentId = payment.id;
    }
    pushAdminNotification(data, {
      type: "payment_paid",
      title: "Payment received (Stripe)",
      message: `Stripe payment completed for order ${order?.id || orderId}`,
      details: {
    ...
    pushAuditLog(data, {
      ...
      action: "payment_paid",
      ip: clientIp(req),
      targetType: payment ? "payment" : "order",
      targetId: payment?.id || order?.id || orderId || sessionId,
      details: { stripeSessionId: sessionId, orderId: order?.id || orderId },
    });
    writeData(data);
    ...
    if (payment) {
      ...
      payment.updatedAt = new Date().toISOString();
      appendEvent(payment, "expired", { orderId: order?.id || orderId });
      writeData(data);
    }
  } else {
    // Store unknown events for later debugging if a payment exists.
    if (payment) {
      appendEvent(payment, "event", {});
      payment.updatedAt = new Date().toISOString();
      writeData(data);
    }
  }
...
app.get("/api/me", requireAuth, (req, res) => {
  ...
  res.json({
    user: {
      ...
      shengen: economyPayload(user).shengen,
    },
  });
});

app.get("/api/economy/balance", requireAuth, (req, res) => {
  res.json({ economy: economyPayload(req.user) });
});

app.post("/api/economy/heartbeat", requireAuth, (req, res) => {
  const data = readData();
  const user = data.users.find((u) => u.id === req.user.id);
  if (!user) return res.status(404).json({ error: "User not found" });
  const earned = applyEconomyHeartbeat(user, Date.now());
  writeData(data);
  res.json({ earned, economy: economyPayload(user) });
});
...
app.post("/api/wheel/spin", requireAuth, (req, res) => {
  ...
  if (nextAt && now < nextAt) {
    ...
    return res.status(429).json({ error: `Wheel cooldown until ${nextSpinAt}`, nextSpinAt });
  }
  const prize = wheelPickPrize();
  const reward = wheelRewardForAmount(prize.amount);
...
app.patch("/api/tickets/:id", requireAuth, (req, res) => {
  ...
  if (prevStatus !== nextStatus) {
    ...
    ticket.statusHistory.unshift({ at: nowIso, by: req.user.id, from: prevStatus, to: nextStatus });
    if (ticket.statusHistory.length > 50) ticket.statusHistory.splice(50);
  }

  const assignedTo = String(req.body?.assignedTo || "").trim();
  if (assignedTo) {
    ticket.assignedTo = assignedTo === "me" ? req.user.id : assignedTo;
  }

  const note = sanitizeText(req.body?.note, 500);
  if (note) {
    ticket.internalNotes.unshift({ at: nowIso, by: req.user.id, text: note });
...
app.post("/api/access-requests", requireAuth, (req, res) => {
  ...
  pushAuditLog(data, {
    ...
    details: { resource: accessRequest.resource },
  });
  pushAdminNotification(data, {
...
app.patch("/api/access-requests/:id", requireAuth, (req, res) => {
  ...
  const prevStatus = String(accessRequest.status || "pending");
...
app.post("/api/orders", requireAuth, async (req, res) => {
  ...
  if (!game || !playerId || !allowedWallets.has(wallet)) {
    return res.status(400).json({ error: "Missing required fields" });
  }
  if (!Number.isFinite(amount) || amount <= 0) return res.status(400).json({ error: "Invalid amount" });
  const isOnlineCard = wallet === "online_card";
  if (!isOnlineCard) {
    if (!sender || !paymentRef || !proofUrl) return res.status(400).json({ error: "Missing required fields" });
  ...
  const order = {
    ...
    amount,
    wallet,
    sender: isOnlineCard ? null : sender,
    paymentRef: isOnlineCard ? null : paymentRef,
    proofUrl: isOnlineCard ? "" : proofUrl,
    status: isOnlineCard ? "pending_payment" : "pending",
    createdAt: new Date().toISOString(),
    userId: req.user.id,
  };
  ...
  pushAuditLog(data, {
    ...
    targetType: "order",
    targetId: order.id,
    details: { game: order.game, amount: order.amount, wallet: order.wallet },
  });
  pushAdminNotification(data, {
    type: "order_created",
  ...
  let payment = null;
  if (isOnlineCard && PAYMENTS_ENABLED && PAYMENTS_PROVIDER === "stripe") {
    try {
      const session = await createStripeCheckoutSession({ order, user: req.user });
      payment = {
        id: crypto.randomUUID(),
      ...
      pushAuditLog(data, {
        ...
        targetType: "payment",
        targetId: payment.id,
        details: { orderId: order.id, provider: "stripe" },
      });
    } catch (error) {
      pushAuditLog(data, {
        actorId: req.user.id,
        ...
        targetType: "order",
        targetId: order.id,
        details: { error: error.message || "stripe error" },
      });
    }
  }
  writeData(data);
  res.json({ order, payment, checkoutUrl });
});
...
app.patch("/api/orders/:id", requireAuth, (req, res) => {
  ...
  if (hasStatus) {
    ...
    if (!["pending", "approved", "rejected", "paid", "pending_payment"].includes(nextStatus)) {
      return res.status(400).json({ error: "Invalid status" });
    }
    const prevStatus = String(order.status || "pending");
    order.status = nextStatus;
    if (prevStatus !== nextStatus) {
      order.statusHistory.unshift({ at: nowIso, by: req.user.id, from: prevStatus, to: nextStatus });
  ...
  const assignedTo = String(req.body?.assignedTo || "").trim();
  if (assignedTo) {
    order.assignedTo = assignedTo === "me" ? req.user.id : assignedTo;
  }
  const note = sanitizeText(req.body?.note, 500);
  if (note) {
    order.internalNotes.unshift({ at: nowIso, by: req.user.id, text: note });
...
app.post("/api/payment-receipts", requireAuth, (req, res) => {
  ...
  const receipt = {
    ...
    userId: req.user.id,
    method,
    receiver: sanitizeText(req.body?.receiver, 120),
    sender: sanitizeText(req.body?.sender, 120),
    amount,
    paymentRef: sanitizeText(req.body?.paymentRef, 200),
    note: sanitizeText(req.body?.note, 500),
    proofUrl: String(req.body?.proofUrl || "").trim().slice(0, 400),
    createdAt: new Date().toISOString(),
    status: "pending",
  };
  ...
  pushAuditLog(data, {
    ...
    targetType: "receipt",
    targetId: receipt.id,
    details: { amount: receipt.amount, method: receipt.method, paymentRef: receipt.paymentRef },
  });
  pushAdminNotification(data, {
...
app.get("/api/audit", requireAuth, requireStaff, (req, res) => {
  const data = readData();
  const limitRaw = Number(req.query?.limit || 200);
  const limit = Math.max(1, Math.min(500, Number.isFinite(limitRaw) ? Math.floor(limitRaw) : 200));
  const items = Array.isArray(data.auditLog) ? data.auditLog.slice(0, limit) : [];
  res.json({ audit: items, limit });
});
...
const closeNotificationStreams = () => {
  notificationStreams.forEach((stream) => {
    try {
      stream.write("event: shutdown\ndata: server shutting down\n\n");
      stream.end();
    } catch (_) {}
  });
...
const shutdown = (signal) => {
  ...
  isShuttingDown = true;
  console.log(`Shutdown signal received (${signal || "unknown"}), closing server...`);
